...
const MainChat: React.FC = () => {
  const { currentTurns, isWaiting } = useChatContext();
  ...
  useEffect(() => {
    scrollToBottom();
  }, [currentTurns, isWaiting]);

  return (
    <div className="max-w-5xl mx-auto p-4 md:p-8 flex flex-col gap-6 pb-32">
      {currentTurns.length === 0 && !isWaiting ? (
        <div className="flex-1 flex items-center justify-center text-on-surface-variant min-h-[50vh]">
          <div className="text-center animate-fade-in-up">
            <h2 className="text-3xl font-display font-light mb-4 text-on-surface tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-primary via-on-surface to-secondary">
        ...
        currentTurns.map((msg, idx) => {
          const isLastTurn = idx === currentTurns.length - 1;
          return (
            <div key={msg.id} className="w-full flex flex-col gap-2">
              ...
              <ArenaResponse
                ...
                judge={msg.judge}
                isLoading={isLastTurn && isWaiting && !msg.solution_1}
              />
            </div>
          );
        })
